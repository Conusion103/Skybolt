   ...
   main.innerHTML = `
      <section class="p-6">
          <h2 class="text-2xl font-bold text-green-600 mb-4">
              Hola Admin, estás editando owners: ${locaL.get("active_user").full_name}
          </h2>

          <input type="text" id="owner-search" placeholder="Buscar por correo..."
    ...
    <div class="bg-white p-4 rounded-lg shadow-md w-full max-w-md mx-auto transform">
        ...
        <form id="edit-owner-form" class="space-y-3 text-sm">
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg max-w-lg w-full">
                <h3 class="text-xl font-bold text-green-600 mb-4">Detalles del Owner</h3>
...
`;
  ...
  const renderOwners = (data) => {
    ...
    if (onlyOwners.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-gray-500">No hay usuarios con rol <strong>owner</strong>.</td></tr>`;
      return;
    }
    ...
    onlyOwners.forEach((owner) => {
      ...
      row.innerHTML = `
        ...
        <td class="px-4 py-2 text-center">
            <button class="btn-view bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 transition" data-id="${owner.id_user}">Ver más</button>
            <button class="btn-edit bg-yellow-400 text-white px-3 py-1 rounded hover:bg-yellow-500 transition" data-id="${owner.id_user}">Editar</button>
            <button class="btn-delete bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition" data-id="${owner.id_user}">Eliminar</button>
        </td>
      `;
  ...
  document.getElementById("edit-owner-form").addEventListener("submit", (e) => {
    ...
    try {
      ...
      // Validaciones con generalFormat
